...
smax_seed = smin + 5   # Seed around which to try maximum particle sizes (radius in microns)
...
def mkifplot(graph_dir, given_theta, reflectances, alt, smin, smax_seed, smax_tests):
    # Name of plot needs to contain altitude to distinguish it
    alt_path = os.path.join(graph_dir, f"smin{smin}_smax{smax_tests[0]}-{smax_tests[-1]}_alt{alt}_varangle.png")
...
if __name__ == "__main__":
    ...
    graph_dir = utils.create_dirpath("smax_varwavel_encleadus", dirs_removed=1)
    ...
    # Setup values to test for maximum particle sizes. Everywhere from smin to 15, except the seed.
    ...
    conditions_dir = f"smin{smin}_smax{smax_tests[0]}-{smax_tests[-1]}_varwavel"
    ...
    # Loop through each altitude. We will use a separate graph for each. Lower altitudes should be more accurate.
    for idx, alt in enumerate(given_altitudes):
        # Compute the nominal (unnormalized) particle size distribution for this altitude, at seed smax
        radii=np.linspace(smin,smax_seed,nsize)
        dr=radii[1]-radii[0]
        diameters=radii*2
        reflectances = [] # Empty array to hold reflectances
        
        # Perform calculations with the seed maximum size. This will allow us to fix a value of tau.
        ...
        # Get initial reflectance data & seed smax data
        ...
        # Loop through maximum particle size tests
        ...
        # Make plot
        mkifplot(os.path.join(graph_dir, conditions_dir), given_wavelengths, reflectances, alt, smin, smax_seed, smax_tests)
