...
smin_seed = 5  #Seed around which to try different minimum particle sizes (radius in microns)
...
def mkifplot(given_theta, reflectances, smin_seed, smin_tests):
    ...
    # Plot seed minimum particle size
    RMSE_seed = RMSE(reflectances[0], reflectances[1])
    ifplt.plot(given_theta, reflectances[1], color='red', label=f'Computed (smin={smin_seed}, RMSE={RMSE_seed:.3e})')
    iflogplt.plot(given_theta, reflectances[1], color='red', label=f'Computed (smin={smin_seed}, RMSE={RMSE_seed:.3e})')

    # Plot each minimum particle size
    for idx, smin in enumerate(smin_tests):
        RMSE_smin = RMSE(reflectances[0], reflectances[idx + 2])
        ifplt.plot(given_theta, reflectances[idx + 2], label=f'Computed (smin={smin}, RMSE={RMSE_smin:.3e})')
        iflogplt.plot(given_theta, reflectances[idx + 2], label=f'Computed (smin={smin}, RMSE={RMSE_smin:.3e})')
    ...
    graph_dir = utils.create_dirpath("smin_varangle_gring", dirs_removed=1)
    # Save to file
    plt.savefig(os.path.join(graph_dir, f"smin{smin_tests[0]}-{smin_tests[-1]}_smax{smax}_varangle.png"))

if __name__ == "__main__":
    # Compute the nominal (unnormalized) particle size distribution around powerlaw ss
    radii=np.linspace(smin_seed,smax,nsize)
    dr=radii[1]-radii[0]
    diameters=radii*2
    ...
    # Perform calculations with the seed minimum size. This will allow us to fix a value of tau.
    sizedists=radii**(-1 * powlaw)
    #Compute the Geometric optical depth of the nominal particle size distribution for this minimum size
    ...
    # Setup values to test for minimum particle sizes. Everywhere from 0.1 to smax, except the seed.
    smin_tests = [0.1]
    smin_tests.extend([i for i in range(1, smax + 1)])
    smin_tests.remove(smin_seed)

    # Iterate through each test
    for smin in smin_tests:
        radii=np.linspace(smin,smax,nsize)
        diameters=radii*2
        sizedists=radii**(-1 * powlaw) # Get size distribution for this minimum size
        ...
        print(f"Added min size {smin:.1f}")

    # Make plot
    mkifplot(given_theta, reflectances, smin_seed, smin_tests)
