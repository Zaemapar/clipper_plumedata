...
smin = 7  #Minimum particle size (radius in microns)
smax = 17   #Maximum particle size (radius in microns)
...
def mkifplot(given_theta, reflectances, powlaw_seed, powlaw_tests, MIN_RMSE=False):
    RMSEs = []
    ...
    graph_dir = utils.create_dirpath("powlaw_varangle_gring", dirs_removed=1)
    # Save to file
    plt.savefig(os.path.join(graph_dir, f"powlaw{powlaw_tests[0]}-{powlaw_tests[-1]}_angle_test.png" if not MIN_RMSE else "model_gring.png"))

    if not MIN_RMSE:
        RMSE_min = min(RMSEs)
        powlaw_idx = RMSEs.index(RMSE_min) - 1
        model_accurate = reflectances[powlaw_idx + 2]
        mkifplot(given_theta, [reflectances[0], model_accurate], (powlaw_tests[powlaw_idx] if powlaw_idx >= 0 else powlaw_seed), [], MIN_RMSE=True)
...
if __name__ == "__main__":
    ...
    #Compute the Geometric optical depth of the nominal particle size distribution for this power law
    ...
    for powlaw in powlaw_tests:
        sizedists=radii**(-1 * powlaw) # Get size distribution for this power law
        # Use optical depths to recompute reflectances at given angles, to compare with data table
        _, _, _, tau_reflectances = get_reflectances(0, 180, wavel, diameters, sizedists, m, tau)
        ...
        print(f"Added power law {powlaw:.1f}")

    # Make plot
    mkifplot(given_theta, reflectances, powlaw_seed, powlaw_tests)
